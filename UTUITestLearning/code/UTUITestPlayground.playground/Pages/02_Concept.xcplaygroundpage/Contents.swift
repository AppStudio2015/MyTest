 ...
 * 概念
 ...
 ### Mock和Stub函数
   * 概念
     ...
     - Stub: 在测试过程中，用来代替某些代码或方法,测试函数调用了一个未编写的函数，可以编写桩函数来代替该被调用的函数，桩代码也用于实现测试隔离。
 
   * Stub和Mock区别与联系
     - Stub是指人为地让一个对象对某个方法返回我们事先规定好的值。Stub运用的主要场景是你需要和别的开发人员协同开发时，别人的模块尚未完成，而你的模块需要用到别人的模块，这时就需要Stub。例如，后端的接口未完成，你的代码已经完成了，Stub可以伪造一个调用的返回。
     - Mock是一个非常容易和stub混淆的概念，简单来说，我们可以将Mock看做是一种更全面和更智能的Stub。明确来说，Mock其实是一个对象，它是对现有类行为的一种模拟（或是对现有接口实现的模拟）。
     -  Mock和Stub最大的区别在于Stub只是简单的方法替换，不涉及新的对象，被Stub的对象可以是业务代码中真正的对象，而Mock行为本身产生新的（不可能在业务代码中出现）的对象，并遵循类的定义响应某些方法。 Mock让你可以检查某种情况下，一个方法是否被调用，或者一个属性是否被正确设值。Objc下可以使用OCMock来mock对象。而Swift的runtime比较弱，所以，Swift上一般要手动写mock。
